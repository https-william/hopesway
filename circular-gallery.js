...
function createTextTexture(gl, text, font = '600 22px Outfit, sans-serif', color = '#1C2B2B') {
...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#1C2B2B', font = '600 22px Outfit, sans-serif' }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
export class CircularGalleryApp {
  ...
  createMedias(items, bend = 2.4, textColor, borderRadius, font) {
  ...
  onTouchMove(e) {
    ...
    const distance = (this.start - x) * (this.scrollSpeed * 0.024);
  ...
  onTouchUp(e) {
    if (this.isDown) {
      ...
      const dist = Math.hypot(endX - this.start, endY - this.startY);
      const duration = Date.now() - this.startTime;
      if (dist < 10 && duration < 380) {
        this.handleCardTap(endX, endY);
      }
    }
    this.isDown = false;
  ...
  handleCardTap(clientX, clientY) {
    ...
    // Find media closest to the tap point
    let closestMedia = null;
    let minDistance = Infinity;

    this.medias.forEach(media => {
      // Project media world X to screen pixels
      const screenX = ((media.plane.position.x) / (this.viewport.width / 2)) * (rect.width / 2) + rect.width / 2;
      const cardWidthPx = (media.plane.scale.x / this.viewport.width) * rect.width;
      const dist = Math.abs(clickX - screenX);
      if (dist < cardWidthPx * 0.6 && dist < minDistance) {
        minDistance = dist;
        closestMedia = media;
      }
    });
    ...
    if (closestMedia) {
      if (this.onCardClick) {
        this.onCardClick(closestMedia);
      } else if (closestMedia.link) {
        if (closestMedia.link.startsWith('http')) {
          window.open(closestMedia.link, '_blank', 'noopener,noreferrer');
        } else {
  ...
  scrollNext() {
    const step = this.medias && this.medias[0] ? this.medias[0].width : 3.5;
